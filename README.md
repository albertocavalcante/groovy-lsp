...
Requirements: `gh` (authenticated for the repo) and `codex` CLI. The tool asks Codex to return a pretty-printed XML block inside ```xml fences; use `--comment` to post the output back to the GitHub issue instead of printing it locally. The default model is `gpt-5-codex`.
