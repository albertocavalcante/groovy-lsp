...
cmd_build() {
    ...
    # In CI, we might want to force a platform or build all. 
    # For now, we mimic the original workflow's single-platform-per-run approach 
    # but allow the caller to handle matrix logic. 
    # If running locally, we produce one JAR for the current platform.
    ...
    # Move VSIX to dist
    local VSIX_FILE=$(find . -maxdepth 1 -name "*.vsix" | head -n 1)
    if [[ -f "$VSIX_FILE" ]]; then
